<template>
  <div v-if="selectedChat" class="message">
                  ...
                  <div :class="item.self ? 'content-main-user-right' : ''">
                    ...
                    <div class="content" :class="(item.msgItem_type == 8 || item.msgItem_type == 12) ? 'card' : ''"
                      ...
                      <div v-if="item.msgItem_type == 0 && !isImage(item.content.text)" class="text"
                        v-html="replaceFace(item.content.text.replace(/\n/g, '<br/>'))"></div>
                      ...
                      <el-image v-if="item.msgItem_type == 2" style="width: 166px" :src="chatUrl + selectedChat.list_id + '/' + item.content.url
                        " :preview-src-list="[
    chatUrl + selectedChat.list_id + '/' + item.content.url,
                      ...
                      <div v-if="item.self && showTip" class="read-state">
                        <span  style="color:#07c427;" v-if="item.read">已读</span>
                        <span style="color:#0909fc;" v-if="!item.read">未读</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <el-tab-pane v-if="group_info_all.is_action > 0" label="群管理" name="3">
              <div class="g_list">
                <div class="g_item">
                  <div>全员禁言</div>
                  <div class="align-center">
                    <el-switch v-model="group_info.is_msg" @change="changeIsMsg">
                    </el-switch>
                  </div>
                </div>
                <div class="g_item">
                  <div>群内禁止加好友</div>
                  <div class="align-center">
                    <el-switch v-model="group_info.can_add_friend" @change="changeAddFriend">
                    </el-switch>
                  </div>
                </div>
