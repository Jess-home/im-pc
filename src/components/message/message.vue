                ...
                <div :class="item.self ? 'content-main-right' : 'content-main-left'">
                  <div :class="item.self ? 'content-main-user-right' : ''">
                    ...
                    <div class="content" :class="(item.msgItem_type == 8 || item.msgItem_type == 12) ? 'card' : ''"
                      ...
                      <div v-if="item.msgItem_type == 0 && !isImage(item.content.text)" class="text"
                       :class="{
                          'large-emoji': isPureEmoji(item.content.text),
                          'single-emoji': isSingleEmoji(item.content.text)
                        }"
                        v-html="replaceFace(item.content.text)"></div>
...
export default {
  ...
  methods: {
    ...
    isPureEmoji(text) {
			...
			// 判断是否为单个表情
			isSingleEmoji(text) {
			if (!this.isPureEmoji(text)) return false;
			
			// 使用spread operator计算emoji数量（正确处理多字节emoji）
			const emojiCount = [...text.trim()].length;
			return emojiCount === 1;
			},
...
/* 纯表情显示（多个表情） */
.large-emoji
  background-color #18d61500 !important
