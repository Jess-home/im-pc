...
<template>
  ...
  >
    <div class="emoji">
      <!-- TODO 对话框功能 -->
      <i
        class="iconfont iconIMweb_expression upload-file-icon"
        :title="$t('xuan_ze_biao_qing')"
        :style="{
      ...
      <!-- <i
        ...
        @click="sendClick"
      ></i> -->
      <i class="iconfont iconIMweb_picture upload-file-icon" 
        :title="$t('fa_song_tu_pian')"
      @click="uploadImage">
        <input
        id="upload_file"
        type="file"
      ...
      <i class="iconfont iconIMweb_file upload-file-icon" 
      :title="$t('fa_song_wen_jian')"
      @click="uploadFile">
        <input
        id="upload_file1"
        type="file"
        class="upload-file"
        @change="fileChange1($event)"
        ref="upload_file1"
      />
      </i>
      <i class="iconfont iconIMweb_video upload-file-icon" 
      :title="$t('fa_song_shi_pin')"
      @click="uploadVideo">
        <input
        id="upload_file2"
        type="file"
      ...
      <!-- <i  class="iconfont iconIMweb_ upload-file-icon" 
      title="@"
      :style="{
      ...
      <i :title="$t('fen_xiang_qun_liao')" class="iconfont iconfont iconIMweb_qcard upload-file-icon" 
      @click="getChatList"
      :style="{
      ...
      <transition name="showbox">
        <div class="concat-content" v-show="show_concat">
          ...
          <div class="concat-input">
            <el-input
              :placeholder="$t('qing_shu_ru_ming_cheng')"
...
import { textMsgs, upload, setListTime,getChatDetails,getFriendList,sendCard,getChatList,sendGroupCard,getMemberPhotos } from '@/api'
  ...
  watch: {
    ...
    selectedList: {
      handler(newval, oldval) {
        ...
        this.selectedChat.list_id = arr.join(",");
       },
      immediate:true
    },
    // 在选择其它对话的时候 聚焦输入框
              ...
              img.onload = function () {
                ...
                upload({
                  ...
                  if (res.err == 0) {
                    ...
                    textMsgs({
                      list_ids: _this.selectedChat.list_id,
                      _token: localStorage.getItem('token'),
                      content_type: 0,
  ...
  methods: {
	...
	// console.log(this.content);return;	
      ...
      if (this.content.length < 1) {
        ...
        textMsgs({
          list_ids: this.selectedChat.list_id,
          _token: localStorage.getItem('token'),
          content_type: 0,
          content: JSON.stringify({
            text: this.content
